...
The integrated model is easily run with the `runModel` function as seen below by supplying the occurrence and *R-INLA* objects (created above with `bruSDM`*)*. We specify `control.inla = list(int.strategy = 'eb')` in our `options` argument in order to speed up the model running time.
