---
title: "Example of cSDMs for the solitary tinamou"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Example of cSDMs for the solitary tinamou}
...
This vignette illustrates the various functions of *cSDMs* by using three datasets of the solitary tinamou (T*inamus solitarius) --* a species of ground bird found on the eastern side of Brazil.
...
`Region` is a `SpatialPolygons` object which give the boundary of the park containing the species; it was used in the *mesh* construction and for the plots in this vignette.
...
The next object is `covariates`, a `list` of `Raster` objects of the covariates (*Forest, NPP* and *Altitude*) describing the area of the parks. We stack these three objects together using the `stack` function, and then scale them.

```{r look at covariates, fig.width=8, fig.height=5}
...
```{r look at mesh, fig.width=8, fig.height=5}
...
```{r plots of data, fig.width=8, fig.height=5}
...
To run the integrated model, we use the `runModel` function with the `data` argument as the object created with the `intModel` function above.
...
Spatial fields are fundamental in our spatial species distribution models, and so we include them in the model by setting `pointsSpatial = TRUE` in `intModel`. For this model, we also remove the intercept terms from the components by setting `pointsIntercept = FALSE`.

```{r set up model with fields}

fields <- intModel(datasets, spatialCovariates = covariates, Coordinates = c('X', 'Y'),
                 Projection = projection, Mesh = mesh, 
                 pointsSpatial = TRUE, pointsIntercept = FALSE)
...
fields$priorsFixed(Effect = 'Forest', mean.linear = 0.5, prec.linear = 0.01)

```

Finally we run the integrated model, again with `runModel` but this time we specify options with *R-INLA*'s *empirical Bayes* integration strategy to help with computation time.
...
```{r plot spatial, fig.width=8, fig.height=5}

plot(spatial_predictions)

```

However if we wanted to make predictions of the bias field, we would do this by setting `biasfield = TRUE` (and then adding `biasnames = 'ebird'`).
...
```{r plot bias, fig.width=8, fig.height=5}
