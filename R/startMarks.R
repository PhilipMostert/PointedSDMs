#' @title \emph{startMarks}: Function used to initialize a marked-point process model.
#' 
...
#' 
#' @return A \code{\link{specifyMarks}} object (class \code{R6}). Use \code{?specifyMarks} of \code{.$help()} to get a comprehensive description of the slot functions associated with this object.
#' 
...
startMarks <- function(..., spatialCovariates = NULL,
                     ...
                     Offset = NULL, pointsSpatial = 'copy',
                     responseCounts = 'counts', responsePA = 'present', trialsPA = NULL,
                     trialsMarks = NULL, temporalName = NULL,
  ...
  if (is.null(markNames)) stop('markNames needs to be non-null.')
  
  if (is.null(markFamily)) {
    
    warning('Mark families not given. Will assume marks as gaussian.')
    markFamily <- rep('gaussian', length(markNames))
    
  }
  
  if (length(markFamily) != length(markNames)) stop('markFamily must be the same length as markNames.')
